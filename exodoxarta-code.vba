...
Const mainSheetName As String = "Φύλλο1"
Const mesimeriana As String = "ΜΕΣΗΜΕΡΙΑΝΑ ΕΞΟΔΟΧΑΡΤΑ"
Const prohna As String = "ΠΡΩΙΝΑ ΕΞΟΔΟΧΑΡΤΑ"
...
Sub dd()
   ...
   sheetCurrentRow = 1
   If Not sheetExists(mesimeriana) Then
        CreateSheet (mesimeriana)
        tmp = WriteToSheet(sheetCurrentRow, "ΕΠΙΘΕΤΟ", "ΟΝΟΜΑ", "ΑΠΟ", "ΜΕΧΡΙ", "ΩΡΑ-ΕΞΟ", "ΩΡΑ-ΜΕΣΑ")
   Else
   ...
   currentday = day(Date)
    ...
    Do While Cells(currentRow, 1).Value <> ""
        ...
        If currentDayStatus = "ΔΙΕ" Then
            ...
            Loop
        ElseIf currentDayStatus = "ΒΑΡ" Then
            Do While innerLoopIndex < endOfLoopMax
                ...
                innerLoopIndex = innerLoopIndex + 1
            Loop
        ElseIf currentDayStatus = "ΥΠ" And epistasia = "ΜΑΓΕΙΡΙΑ" Then
            Do While innerLoopIndex < endOfLoopMax
                If HasDuty(currentRow, innerLoopIndex, endOfLoopMax - 1, innerLoopIndex - loopOffset) Then
                    'Debug.Print currentDay & "  " & innerLoopIndex - loopOffset
                    sheetCurrentRow = sheetCurrentRow + 1
...
Public Function HasDuty(currentRow As Integer, innerLoopIndex As Integer, lastDayOfMonthCell As Integer, currentColumn As Integer) As Boolean
    If Trim(Cells(currentRow, innerLoopIndex).Value) = "ΔΙΕ" Or Trim(Cells(currentRow, innerLoopIndex).Value) = "ΣΚ" Or Trim(Cells(currentRow, innerLoopIndex).Value) = "ΥΠ" Or Trim(Cells(currentRow, innerLoopIndex).Value) = "ΒΑΡ" Then
        HasDuty = True
    Else
        If Trim(Cells(currentRow, lastDayOfMonthCell).Value) = "ΕΞΟ" And lastDayOfMonthCell = currentColumn Then
            HasDuty = True
        Else
            HasDuty = False
        End If
